<template>
  <div class="file_item">
    <div class="file_label" style="display: flex; justify-content: center;">
      <Checkbox v-model="item.selected" binary size="small" @change="()=>selector().selectChange(item)" @mousedown.stop @click.stop />
    </div>
    <div @click="clickHanlder(item)" @mousedown.stop @click.stop>
      <FileIcon :file="item" />
    </div>
    <div class="file_label" @click="clickHanlder(item)" @mousedown.stop @click.stop>{{ item.name }}</div>
    ...
    <div class="file_label" @click="clickHanlder(item)" @mousedown.stop @click.stop style="text-align: right;">{{ calSize(item) }}</div>
